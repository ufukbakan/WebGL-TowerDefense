...
function placeRotators(scene) {
    ...
    rotatorPositions.forEach(position =>{
        ...
        let rotator = new Mesh(new BoxGeometry(0.5, mesh_height, 0.5), new MeshBasicMaterial({ color: 0x00ff00 }));
 ...
 */
 function placeForbiddenPath(scene) {
    ...
    let forbiddenPath1 =  new Mesh(new BoxGeometry(16,mesh_height,1), new MeshBasicMaterial({color: 0xff0000}));
    forbiddenPath1.position.set(0, mesh_height/2, -2);

    let forbiddenPath2 =  new Mesh(new BoxGeometry(1,mesh_height,16), new MeshBasicMaterial({color: 0xff0000}));
    forbiddenPath2.position.set(2, mesh_height/2, 0);

    let forbiddenPath3 =  new Mesh(new BoxGeometry(16,mesh_height,1), new MeshBasicMaterial({color: 0xff0000}));
